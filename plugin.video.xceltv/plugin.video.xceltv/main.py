...
try:
    if len(sys.argv) >= 3:
        ...
        client = Server('http://localhost:%d/xceltv' % 11421)
        ...
        addon_context = AddonContext(addon_id='plugin.video.xceltv', conf={'contextFiles':['actions.xml', 'dr_actions.xml', 'dtf_actions.xml'], 'webServiceEnabled':True, 'webServicePath':'/xceltv', 'webServicePort':11421})
